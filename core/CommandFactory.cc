...
RichQuery CommandFactory::generateQuery(const std::string &command, const std::string &regexp, const std::string &pattern, const std::string &arg, const char target) {
	std::vector<std::string> v;
	v.push_back(arg);
...
RichQuery CommandFactory::generateQuery(const std::string &command, const std::string &regexp, const std::string &pattern, const std::vector<std::string> &args, const char target) {
	return RichQuery(replace(command, target, args), regexp, pattern);
}

std::string CommandFactory::generateAnswer(const std::string &answer, const std::string &regexp, const std::string &pattern, const char target) throw(std::invalid_argument) {
	std::regex rgx;
	try {
		rgx = std::regex(regexp);
	} catch(...) {
		throw std::invalid_argument(ExceptionFactory::generateMessage("Regular Expression \'" + regexp + "\' does not compile!", "CommandFactory.cc", __LINE__));
	}
...
std::string CommandFactory::replace(std::string pattern, const char target, const std::vector<std::string> &args) {
	...
	for (size_t i=0; i<pattern.length(); i++) {
		if (c >= args.size()) out += pattern[i];
		else if (pattern[i] == target) qmark = true;
		else {
			if (qmark) {
